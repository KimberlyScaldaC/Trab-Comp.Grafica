...
public class manager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 24; i++)
        {
            ...
            if (!vetGameObj[i].TryGetComponent<Rigidbody>(out var rb))
            {
                rb = vetGameObj[i].AddComponent<Rigidbody>();
            }
            rb.isKinematic = true;

            if (!vetGameObj[i].TryGetComponent<MeshCollider>(out var mc))
            {
                mc = vetGameObj[i].AddComponent<MeshCollider>();
            }
            mc.convex = true; // Para funcionar com triggers
        }
        ...
        GameObject planoTrigger = new GameObject("PlanoTrigger");
        BoxCollider box = planoTrigger.AddComponent<BoxCollider>();
        box.size = new Vector3(3, 0.01f, 3); // largura, altura, profundidade
        box.isTrigger = true;
        planoTrigger.transform.position = new Vector3(1.5f, 0.432f, 1); // altura no meio dos tetraedros
        planoTrigger.AddComponent<PlaneColliderDetector>();
